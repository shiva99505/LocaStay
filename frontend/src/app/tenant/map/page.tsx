...
export default async function MapMyStayPage() {
  ...
  const [{ data: activeBooking }, { data: nearbyProperties = [] }] = await Promise.all([
    supabase
      .from('bookings')
      .select('*, property:properties!property_id(latitude, longitude, city, state)')
    ...
    supabase
      .from('properties')
      .select('id, title, city, state, rent, latitude, longitude, is_verified, status')
  ...
  const mapProperties: MapProperty[] = (nearbyProperties ?? []).map((p) => ({
    ...p,
