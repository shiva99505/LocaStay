...
export default async function ComplaintsPage() {
  ...
  const [{ data: complaints = [] }, { data: maintenanceRequests = [] }] = await Promise.all([
    supabase.from('complaints').select('*').eq('user_id', session.user.id).order('created_at', { ascending: false }),
    supabase.from('maintenance_requests').select('*').eq('tenant_id', session.user.id).order('created_at', { ascending: false }),
  ]);
                  ...
                  <div key={complaint.id} className="px-5 py-4">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">{CATEGORY_LABELS[complaint.category] ?? complaint.category} · {formatDate(complaint.created_at)}</p>
                      </div>
            ...
            {(maintenanceRequests ?? []).map((req) => (
              <div key={req.id} className="flex items-center gap-3 rounded-xl border border-border p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{req.category.toLowerCase()} · {req.status.toLowerCase().replace(/_/g, ' ')} · {formatDate(req.created_at)}</p>
                </div>
              </div>
            ))}
          </CardContent>
