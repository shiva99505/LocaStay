          ...
          {(notifications ?? []).length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {(notifications ?? []).map((notif) => (
                <div key={notif.id} className={cn('flex gap-4 px-5 py-4 transition-colors hover:bg-muted/20', !notif.is_read && 'bg-secondary-50/40 dark:bg-secondary-500/5')}>
                  <div className="pt-1 shrink-0">
                    <div className={cn('h-2.5 w-2.5 rounded-full', notif.is_read ? 'bg-transparent' : 'bg-secondary-600')} />
                  </div>
