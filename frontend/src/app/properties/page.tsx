...
async function getProperties(params: SearchParams): Promise<{ properties: PropertyCardData[]; total: number }> {
  ...
  const where = {
    status: { in: ['AVAILABLE', 'OCCUPIED', 'PENDING'] as string[] },
  ...
  const properties = (raw as Record<string, unknown>[]).map((p) => ({
    id: p.id as string,
    ...
    coverImage: (p.coverImage ?? p.cover_image ?? null) as string | null,
    ...
    isFeatured: (p.isFeatured ?? p.is_featured ?? false) as boolean,
    totalRooms: (p.totalRooms ?? p.total_rooms ?? 0) as number,
    occupiedRooms: (p.occupiedRooms ?? p.occupied_rooms ?? 0) as number,
    nearestDistanceKm: [p.distanceToSchool ?? p.distance_to_school, p.distanceToMarket ?? p.distance_to_market, p.distanceToBusStand ?? p.distance_to_bus_stand]
