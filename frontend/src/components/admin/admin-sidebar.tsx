        ...
        <nav className="space-y-0.5">
          {NAV_ITEMS.map((item) => (
            <NavLink key={item.href} {...item}
              badge={item.href === '/admin/support' ? unreadCount : undefined}
              onClick={onClose}
            />
          ))}
        </nav>
...
export function AdminSidebar({ user, unreadCount }: { user: SidebarUser; unreadCount: number }) {
  return (
...
export function AdminMobileNav({ user, unreadCount }: { user: SidebarUser; unreadCount: number }) {
