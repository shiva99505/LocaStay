...
const NAV_ITEMS = [
  ...
  { href: '/tenant/profile',       label: 'Profile & KYC', icon: User },
  ...
  return (
    <Link href={href} onClick={onClick}
      className={cn('group flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition-colors',
        active ? 'bg-primary-700 text-white' : 'text-muted-foreground hover:bg-muted hover:text-foreground',
      )}
    >
      ...
      {badge != null && badge > 0 && (
        <span className={cn('flex h-5 min-w-[20px] items-center justify-center rounded-full px-1 text-[10px] font-bold',
          active ? 'bg-white/20 text-white' : 'bg-primary-100 text-primary-800 dark:bg-primary-500/20 dark:text-primary-300')}>
          {badge > 99 ? '99+' : badge}
        </span>
      )}
    </Link>
...
function SidebarContent({ user, unreadCount, onClose }: {
  user: SidebarUser; unreadCount: number; onClose?: () => void;
}) {
...
export function TenantSidebar({ user, unreadCount }: { user: SidebarUser; unreadCount: number }) {
  return (
...
export function TenantMobileNav({ user, unreadCount }: { user: SidebarUser; unreadCount: number }) {
