...
import {
  MapPin, Star, ShieldCheck, BedDouble, Calendar,
...
import { fromJsonArray, PROPERTY_TYPE_LABELS, type PropertyType } from '@/lib/constants';
import { formatCurrency, formatDate, formatDistanceKm, googleMapsViewUrl, initials, cn } from '@/lib/utils';
import { PropertyUnlockGate } from '@/components/properties/property-unlock-gate';
...
export default async function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isUnlocked = session?.user?.role === 'TENANT'
    ? (await prisma.propertyUnlock.findUnique({
        where: { tenantId_propertyId: { tenantId: session.user.id, propertyId: id } },
      })) !== null
    : false;

  const images = [property.coverImage, ...fromJsonArray<string>(property.images)].filter(Boolean) as string[];
      ...
      <main className="flex-1">
            ...
            <div className="relative mx-auto max-w-7xl">
              <div className="grid h-72 grid-cols-4 gap-1.5 overflow-hidden rounded-b-none px-0 sm:h-80 lg:h-[420px] lg:rounded-b-2xl">
                <div className="relative col-span-2 row-span-2 overflow-hidden rounded-b-none lg:rounded-bl-2xl">
                  <Image src={images[0]} alt={property.title} fill sizes="50vw" className="object-cover" />
                </div>
                {images.slice(1, 5).map((img, i) => (
                  <div key={i} className={cn('relative overflow-hidden', i === 1 && 'lg:rounded-tr-2xl', i === 3 && 'lg:rounded-br-2xl')}>
                    <Image src={img} alt={`${property.title} photo ${i + 2}`} fill sizes="25vw" className="object-cover transition-transform hover:scale-105" />
                  </div>
                ))}
                {images.length < 2 && Array.from({ length: 4 - Math.max(images.length - 1, 0) }).map((_, i) => (
                  <div key={i} className="flex items-center justify-center bg-muted/60"><BedDouble className="h-8 w-8 text-muted-foreground/30" /></div>
                ))}
              </div>
            </div>
          ) : (
            <div className="flex h-64 items-center justify-center"><BedDouble className="h-16 w-16 text-muted-foreground/30" /></div>
          )}
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
            ...
            <div className="flex-1 space-y-6">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant={vacancies > 0 ? 'info' : 'muted'}>{vacancies > 0 ? `${vacancies} room${vacancies > 1 ? 's' : ''} available` : 'Fully occupied'}</Badge>
                </div>
              ...
              {distances.length > 0 && (
                <Card>
                  <CardHeader><CardTitle className="flex items-center gap-2 text-base font-bold"><MapPin className="h-4 w-4 text-primary-600" /> Nearby Distances</CardTitle></CardHeader>
                  <CardContent>
                    ...
                    <a href={googleMapsViewUrl(property.latitude, property.longitude)} target="_blank" rel="noopener noreferrer"
                      ...
                      <div key={review.id} className="space-y-2">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-8 w-8 rounded-lg">
                            {review.tenant.avatar ? <img src={review.tenant.avatar} alt={review.tenant.name ?? 'Tenant'} className="h-full w-full rounded-lg object-cover" /> : null}
                        ...
                        {review.comment && <p className="text-sm text-foreground/90 pl-11">{review.comment}</p>}
            ...
            {/* Sidebar: Landlord contact + sticky book CTA */}
            <div className="w-full space-y-4 lg:w-80 lg:shrink-0">
              <div className="sticky top-4 space-y-4">
                {/* Book + Contact card — gated behind ₹3 unlock */}
                <Card>
                  <CardContent className="pt-5 space-y-4">
                    <div className="text-center">
                      <p className="font-display text-2xl font-extrabold text-foreground">{formatCurrency(property.rent)}<span className="text-sm font-normal text-muted-foreground">/month</span></p>
                      <p className="text-xs text-muted-foreground mt-0.5">Deposit: {formatCurrency(property.deposit)}</p>
                    </div>
                    <PropertyUnlockGate
                ...
                {/* Landlord info card */}
                <Card>
                  <CardHeader><CardTitle className="text-sm font-bold text-muted-foreground uppercase tracking-wide">Listed by</CardTitle></CardHeader>
                  <CardContent className="space-y-3 pt-0">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-12 w-12 rounded-xl">
                        {property.landlord.user.avatar ? (
                          <img src={property.landlord.user.avatar} alt={property.landlord.user.name} className="h-full w-full rounded-xl object-cover" />
                        ) : null}
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">Member since {formatDate(property.landlord.user.createdAt, { year: 'numeric', month: 'long' })}</p>
                      </div>
                    </div>
