...
export default async function RentTrackerPage() {
  ...
  const paid = payments.filter((p) => p.status === 'PAID').reduce((s, p) => s + p.amount + p.lateFee, 0);
          ...
          <CardContent className="flex flex-col gap-2 pt-5">
            ...
            {overdue.length > 0 ? (
              <p className="text-xs font-medium text-red-600">
                {formatCurrency(overdue.reduce((s, p) => s + p.amount + p.lateFee, 0))} total outstanding
              </p>
            ) : (
          ...
          <CardContent className="flex flex-col gap-2 pt-5">
            ...
            {pending.length > 0 && (
              <p className="text-xs font-medium text-amber-700 dark:text-amber-400">
                Due: {formatDate(pending[0].dueDate)}
              </p>
            )}
          </CardContent>
        </Card>
                    ...
                    return (
                      <tr key={payment.id} className="hover:bg-muted/20">
                        ...
                        <td className="px-4 py-3 text-right font-bold text-foreground">
                          ...
                          {payment.lateFee > 0 && (
                            <span className="ml-1 text-xs font-normal text-red-500">+{formatCurrency(payment.lateFee)} late</span>
                          )}
                        </td>
                          ...
                          <div className="flex items-center justify-center gap-1">
                            {canPay ? (
                              <PayRentButton
                                ...
                                upiId={payment.property.landlord.upiId}
                              />
                            ) : (
