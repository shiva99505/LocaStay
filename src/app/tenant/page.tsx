...
import {
  ...
  Clock, CheckCircle2, XCircle, Building2, ChevronRight,
  Wrench, CreditCard, Home, HelpCircle,
} from 'lucide-react';
...
export default async function TenantDashboard() {
  ...
  const [activeBooking, pendingBookings, notifications, savedCount, recommended] = await Promise.all([
    prisma.booking.findFirst({
      where: { tenantId: session.user.id, status: 'APPROVED' },
      include: {
        property: { select: { id: true, title: true, address: true, city: true, state: true, coverImage: true } },
        payments: { orderBy: { dueDate: 'desc' }, take: 1 },
      },
      orderBy: { requestedAt: 'desc' },
    }),

    prisma.booking.count({ where: { tenantId: session.user.id, status: 'PENDING' } }),

    prisma.notification.findMany({
      where: { userId: session.user.id, isRead: false },
      orderBy: { createdAt: 'desc' },
      take: 5,
      select: { id: true, title: true, message: true, createdAt: true, link: true },
    }),

    prisma.savedProperty.count({ where: { userId: session.user.id } }),

    prisma.property.findMany({
      where: { status: 'AVAILABLE' },
      orderBy: [{ isFeatured: 'desc' }, { rating: 'desc' }],
      take: 4,
      select: { id: true, title: true, city: true, state: true, coverImage: true, rent: true, rating: true, isVerified: true },
    }),
  ]);

  const latestPayment = activeBooking?.payments[0] ?? null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-4">
          <Avatar className="h-14 w-14 rounded-2xl border-2 border-border">
            {session.user.avatar ? <AvatarImage src={session.user.avatar} alt={session.user.name ?? 'Tenant'} /> : null}
            <AvatarFallback className="rounded-2xl text-lg">{initials(session.user.name ?? 'T')}</AvatarFallback>
          </Avatar>
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              {activeBooking ? `Staying at ${activeBooking.property.city}` : 'Find your perfect home today'}
            </p>
          </div>
        </div>
        {rentDue && (
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard
          ...
          label="Active Stay"
          value={activeBooking ? activeBooking.property.city : '—'}
          ...
          <CardContent className="pt-0">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
              <div className="relative h-28 w-full overflow-hidden rounded-xl bg-muted sm:h-24 sm:w-40 sm:shrink-0">
                {activeBooking.property.coverImage ? (
                  <img src={activeBooking.property.coverImage} alt={activeBooking.property.title} className="h-full w-full object-cover" />
                ) : (
                  <Building2 className="absolute inset-0 m-auto h-8 w-8 text-muted-foreground/40" />
                )}
              </div>
              <div className="flex-1 space-y-1.5">
                <p className="font-display text-base font-bold text-foreground">{activeBooking.property.title}</p>
                <p className="flex items-center gap-1.5 text-sm text-muted-foreground">
                  <MapPin className="h-3.5 w-3.5" /> {activeBooking.property.address}, {activeBooking.property.city}, {activeBooking.property.state}
                </p>
              </div>
          ...
          <CardContent className="flex flex-col items-center justify-center gap-4 py-12 text-center">
            ...
            {pendingBookings > 0 && (
              <Button asChild variant="ghost" size="sm" className="gap-1.5 text-muted-foreground">
                <Link href="/tenant/stay"><Clock className="h-4 w-4" /> {pendingBookings} pending booking request{pendingBookings > 1 ? 's' : ''}</Link>
              </Button>
            )}
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { href: '/tenant/support', icon: HelpCircle, label: 'Support', urgent: false },
        ].map((action) => (
          <Link
            key={action.href}
            href={action.href}
            ...
            {recommended.map((p) => (
              <Link key={p.id} href={`/properties/${p.id}`} className="group overflow-hidden rounded-2xl border border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-md">
                <div className="relative h-40 overflow-hidden bg-muted">
                  {p.coverImage ? <img src={p.coverImage} alt={p.title} className="h-full w-full object-cover group-hover:scale-105 transition-transform" /> : <Building2 className="absolute inset-0 m-auto h-8 w-8 text-muted-foreground/40" />}
                  {p.isVerified && <span className="absolute right-2 top-2 flex items-center gap-1 rounded-full bg-green-500/90 px-2 py-0.5 text-[10px] font-bold text-white">✓ Verified</span>}
                </div>
                <div className="p-3">
