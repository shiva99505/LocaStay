...
export default async function LeadsPage() {
  ...
  const grouped = { new: leads.filter((l) => l.status === 'NEW').length, contacted: leads.filter((l) => l.status === 'CONTACTED').length, converted: leads.filter((l) => l.status === 'CONVERTED').length };
              ...
              {leads.map((lead) => (
                <div key={lead.id} className="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{lead.property?.title ?? 'General'} · {SOURCE_LABELS[lead.source] ?? lead.source} · {timeAgo(lead.createdAt)}</p>
