...
export default async function LandlordRentPage() {
  ...
  const payments = await prisma.rentPayment.findMany({
    ...
    include: {
      tenant: { select: { name: true, phone: true, avatar: true } },
          ...
          <CardContent className="pt-5 space-y-2">
            ...
            {totalOverdue > 0 && (
              <BulkRemindButton
                ...
                tenantIds={payments.filter((p) => p.status === 'OVERDUE').map((p) => p.tenantId)}
              />
            )}
          </CardContent>
        </Card>
                  ...
                  {payments.map((payment) => {
                    ...
                    return (
                      <tr key={payment.id} className="hover:bg-muted/20">
                        ...
                        <td className="px-4 py-3 text-muted-foreground">{payment.paidDate ? formatDate(payment.paidDate) : '—'}</td>
