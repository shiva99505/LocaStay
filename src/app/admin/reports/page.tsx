...
export default async function AdminReportsPage() {
  ...
  const [
    userStats, propertyStats, bookingStats,
    revenueData, topProperties, _topLandlords,
  ] = await Promise.all([
    prisma.user.groupBy({ by: ['role'], _count: { id: true } }),
    prisma.property.groupBy({ by: ['status'], _count: { id: true } }),
    prisma.booking.groupBy({ by: ['status'], _count: { id: true } }),
    prisma.rentPayment.aggregate({ where: { status: 'PAID' }, _sum: { amount: true }, _count: { id: true } }),
    prisma.property.findMany({ orderBy: { views: 'desc' }, take: 5, select: { id: true, title: true, city: true, views: true, rent: true, rating: true } }),
    prisma.landlordProfile.findMany({
      take: 5,
      include: { user: { select: { name: true } }, _count: { select: { properties: true } } },
      orderBy: { responseRate: 'desc' },
    }),
  ]);

  const userCounts = Object.fromEntries(userStats.map((u) => [u.role, u._count.id]));
  const propCounts = Object.fromEntries(propertyStats.map((p) => [p.status, p._count.id]));
  const bookingCounts = Object.fromEntries(bookingStats.map((b) => [b.status, b._count.id]));
  const totalRevenue = revenueData._sum.amount ?? 0;
  const totalPayments = revenueData._count.id;
            ...
            {topProperties.map((prop, i) => (
              <div key={prop.id} className="flex items-center gap-3">
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold text-foreground">{prop.views.toLocaleString('en-IN')} views</p>
